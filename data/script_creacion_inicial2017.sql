...
CREATE TABLE GDDIANOS.COCHE_TURNO
(
	COCH_TURN_TURNO_ID INT NOT NULL,
	COCH_TURN_COCHE_PAT VARCHAR(10) NOT NULL,

	CONSTRAINT PK_COCHE_TURNO PRIMARY KEY(COCH_TURN_TURNO_ID,COCH_TURN_COCHE_PAT),
	CONSTRAINT FK_COCH_TURN_TURNO_ID FOREIGN KEY (COCH_TURN_TURNO_ID) REFERENCES GDDIANOS.TURNO(TURNO_ID),
	CONSTRAINT FK_COCH_TURN_COCHE_PAT FOREIGN KEY (COCH_TURN_COCHE_PAT) REFERENCES GDDIANOS.COCHE(COCHE_PATENTE)
...
BEGIN
	...
	SELECT @CANT = COUNT(*) FROM GDDIANOS.TURNO 
	WHERE (@ID != TURNO_ID) AND (TURNO_ESTADO = 1) AND ((@HI BETWEEN TURNO_HORA_INICIO AND TURNO_HORA_FIN) AND (@HF BETWEEN TURNO_HORA_INICIO AND TURNO_HORA_FIN))
...
;
 ...
 INSERT INTO GDDIANOS.VIAJE 
  ...
  INSERT INTO GDDIANOS.COCHE_TURNO
  (
	[COCH_TURN_COCHE_PAT]
	,[COCH_TURN_TURNO_ID])
	SELECT DISTINCT 
       [Auto_Patente]    
	  ,GDDIANOS.GET_ID_TURNO_POR_HORARIO([Turno_Hora_Inicio],[Turno_Hora_Fin]) AS TURNO_ID
  FROM [GD1C2017].[gd_esquema].[Maestra]
  ;