...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.GET_PERSONA_POR_DNI_TELEFONO'))
DROP PROCEDURE GDDIANOS.GET_PERSONA_POR_DNI_TELEFONO
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.FIND_TURNO_PARA_COCHE'))
DROP PROCEDURE GDDIANOS.FIND_TURNO_PARA_COCHE
...
(
	...
	CONSTRAINT PK_PAGO PRIMARY KEY(PAGO_FECHA,PAGO_PERS_ID) 
);

CREATE TABLE GDDIANOS.ITEM_PAGO
(
	ITEM_PAGO_FECHA DATETIME NOT NULL,	
	ITEM_PAGO_VIAJE_ID INT NOT NULL,
	ITEM_PAGO_VIAJE_IMPORTE FLOAT NOT NULL

	--FALTAN LAS FK
	CONSTRAINT PK_ITEM_PAGO PRIMARY KEY(ITEM_PAGO_FECHA,ITEM_PAGO_VIAJE_ID)
);
...
(
	...
	TURNO_TURNO_ESTADO BIT NOT NULL
...
ITEM_PAGO_VIAJE_ID INT,ITEM_PAGO_VIAJE_IMPORTE FLOAT);
...
GO
CREATE PROCEDURE GDDIANOS.GET_PERSONA_POR_DNI_TELEFONO(@DNI INT, @TELEFONO INT) AS
BEGIN
	SELECT PERS_ID,PERS_NOMBRE,PERS_APELLIDO
	FROM GDDIANOS.PERSONA PERSONAS
	WHERE PERSONAS.PERS_DNI = @DNI AND 
			PERSONAS.PERS_TELEFONO = @TELEFONO
END
...
AS
BEGIN
	SELECT COCHE_ESTADO, PERS_NOMBRE ++ ' ' ++ PERS_APELLIDO PERSONA
...
GO
CREATE PROCEDURE GDDIANOS.MODIFICAR_AUTOMOVIL_POR_PATENTE (@PATENTE VARCHAR(10), @ESTADO INT, @CHOFER VARCHAR(510))
AS
BEGIN
	DECLARE @CHOFERNUMERO INT
	SET @CHOFERNUMERO = (SELECT TOP 1 PERS_ID
						FROM GDDIANOS.PERSONA
						WHERE PERS_NOMBRE ++ ' ' ++ PERS_APELLIDO = @CHOFER)
	UPDATE GDDIANOS.COCHE SET COCHE_ESTADO = @ESTADO, COCHE_CHOFER = @CHOFERNUMERO WHERE COCHE_PATENTE = @PATENTE
END

GO
CREATE PROCEDURE GDDIANOS.CHOFER_ASIGNADO (@CHOFER VARCHAR(510), @PATENTE VARCHAR(10))
AS
BEGIN
SELECT COCHE_PATENTE
FROM GDDIANOS.COCHE JOIN GDDIANOS.PERSONA ON (COCHE_CHOFER = PERS_ID)
WHERE PERS_NOMBRE ++ ' ' ++ PERS_APELLIDO = @CHOFER AND COCHE_PATENTE != @PATENTE
END

GO
CREATE PROCEDURE GDDIANOS.ALTA_AUTOMOVIL (@MARCA VARCHAR(32), @MODELO VARCHAR(255), @PATENTE VARCHAR(10), @CHOFER VARCHAR(510), @ESTADO BIT)
AS
BEGIN
	DECLARE @IDCHOFER INT
	SET @IDCHOFER = (SELECT PERS_ID FROM GDDIANOS.PERSONA WHERE PERS_NOMBRE ++ ' ' ++ PERS_APELLIDO = @CHOFER)
...
CREATE PROCEDURE GDDIANOS.CREAR_FACTURA (@ID_PERS INT, @FECHA_ACTUAL DATETIME, @FECHA_INICIO DATETIME,
	@FECHA_FINAL DATETIME, @VIAJES GDDIANOS.ITEM_FACT_TABLETYPE READONLY, @TOTAL DECIMAL(18,2)) AS
BEGIN
...
CREATE PROCEDURE GDDIANOS.CREAR_PAGO (@FECHA DATETIME, @ID_CHOF INT, @ID_TURNO INT, 
							@TOTAL FLOAT, @VIAJES AS GDDIANOS.ITEM_PAGO_TABLETYPE READONLY) AS
	BEGIN
			INSERT INTO GDDIANOS.PAGO (PAGO_FECHA, PAGO_PERS_ID, PAGO_TURNO_ID,
			PAGO_IMPORTE_TOTAL)
			VALUES (@FECHA, @ID_CHOF, @ID_TURNO, @TOTAL)
	END
	
	BEGIN
		INSERT INTO GDDIANOS.ITEM_PAGO 
		SELECT @FECHA,ITEM_PAGO_VIAJE_ID,ITEM_PAGO_VIAJE_IMPORTE FROM @VIAJES
	END
...
GO
CREATE PROCEDURE GDDIANOS.FIND_TURNO_PARA_COCHE (@PATENTE VARCHAR(10))
AS
BEGIN
	SELECT TURNO_DESCRIPCION, ISNULL(TURNO_TURNO_ESTADO, 0) TURNO_TURNO_ESTADO
	FROM GDDIANOS.TURNO LEFT JOIN GDDIANOS.TURNO_POR_COCHE ON (TURNO_ID = TURNO_TURNO_ID)
	WHERE TURNO_COCHE_PATENTE = @PATENTE
END

GO
CREATE PROCEDURE GDDIANOS.MODIFICAR_TURNO_AUTOMOVIL_POR_PATENTE (@PATENTE VARCHAR(10), @DETALLE VARCHAR(255), @ESTADOTURNO BIT)
AS
BEGIN
	DECLARE @CANT INT
	DECLARE @TURNOID INT
	...
	FROM GDDIANOS.TURNO JOIN GDDIANOS.TURNO_POR_COCHE ON (TURNO_ID = TURNO_TURNO_ID)
	WHERE TURNO_DESCRIPCION = @DETALLE)

	SET @TURNOID = (SELECT TURNO_ID FROM GDDIANOS.TURNO WHERE TURNO_DESCRIPCION = @DETALLE)

	IF @CANT = 0
	BEGIN
		INSERT GDDIANOS.TURNO_POR_COCHE VALUES (@PATENTE,@TURNOID,@ESTADOTURNO)
	END
	ELSE
	BEGIN
		UPDATE GDDIANOS.TURNO_POR_COCHE SET TURNO_TURNO_ESTADO = @ESTADOTURNO WHERE TURNO_TURNO_ID = @TURNOID
	END
END
...
AS
BEGIN
	DECLARE @FACT_PERS_ID INT;DECLARE @TIPO_PERSONA INT;DECLARE @FACT_ID_INS INT;DECLARE @MES INT;DECLARE @ANIO INT; DECLARE @FACT_FECHA_GENERADA DATETIME;DECLARE @FACT_FECHA_INICIO DATETIME;DECLARE @FACT_FECHA_FIN DATETIME;DECLARE @FACT_IMPORTE_TOTAL DECIMAL(18,2);
	DECLARE FACTURAS_A_INSERTAR CURSOR FOR SELECT * FROM INSERTED
	OPEN FACTURAS_A_INSERTAR FETCH NEXT FROM FACTURAS_A_INSERTAR INTO @FACT_ID_INS,@FACT_FECHA_GENERADA,@FACT_FECHA_INICIO,@FACT_FECHA_FIN,@FACT_IMPORTE_TOTAL,@FACT_PERS_ID
	SELECT @MES = MONTH(@FACT_FECHA_INICIO) FROM INSERTED;
	SELECT @ANIO = YEAR(@FACT_FECHA_INICIO) FROM INSERTED;
	SELECT @TIPO_PERSONA = PERS_TIPO FROM GDDIANOS.PERSONA WHERE PERS_ID=@FACT_PERS_ID;
	IF(@TIPO_PERSONA=1 OR @TIPO_PERSONA=3) 
	BEGIN 
		...
		INSERT INTO GDDIANOS.FACTURA ([FACT_ID]
		  ...
		  ,FACT_FECHA_FIN
		  ,FACT_IMPORTE_TOTAL
		  ,FACT_PERS_ID FROM INSERTED;

		WHILE (@@FETCH_STATUS=0)
		BEGIN
			INSERT INTO GDDIANOS.ITEM_FACTURA --INSERTAR LOS ITEMS DE LA FACTURA
				SELECT @FACT_ID_INS,VIAJE_ID,GDDIANOS.GET_IMPORTE_ITEM_FACTURA(@MES,@ANIO,@FACT_PERS_ID,VIAJE_ID) FROM GDDIANOS.VIAJE 
					WHERE VIAJE_CLIENTE_PERS_ID=@FACT_PERS_ID AND MONTH(VIAJE_FECHA) = @MES AND YEAR(VIAJE_FECHA) = @ANIO
			FETCH NEXT FROM FACTURAS_A_INSERTAR INTO @FACT_ID_INS,@FACT_FECHA_GENERADA,@FACT_FECHA_INICIO,@FACT_FECHA_FIN,@FACT_IMPORTE_TOTAL,@FACT_PERS_ID
		END
...
/*INSERT INTO GDDIANOS.FACTURA ([FACT_ID]
      ...
      ,Factura_Fecha_Fin
	  ,(SELECT SUM(TURNO_PRECIO_BASE + (VIAJE_CANTIDAD_KM * TURNO_VALOR_KM))
			FROM GDDIANOS.VIAJE VIAJE JOIN GDDIANOS.TURNO TURNO ON (VIAJE.VIAJE_TURNO_ID = TURNO.TURNO_ID)
				WHERE MONTH(VIAJE_FECHA) = MONTH(Factura_Fecha_Inicio) AND
					YEAR(VIAJE_FECHA) = YEAR(Factura_Fecha_Inicio) AND VIAJE_CLIENTE_PERS_ID = PERSONA.PERS_ID)
...
( [TURNO_COCHE_PATENTE], [TURNO_TURNO_ID], [TURNO_TURNO_ESTADO])
SELECT DISTINCT 
[COCHE_PATENTE], [TURNO_ID], [TURNO_ESTADO]
FROM GDDIANOS.TURNO T JOIN GD_ESQUEMA.MAESTRA M ON (T.TURNO_DESCRIPCION = M.Turno_Descripcion),
	 ...
	 GROUP BY [COCHE_PATENTE], [TURNO_ID], [TURNO_ESTADO];
