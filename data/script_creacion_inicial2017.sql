...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.BORRAR_PERSONA'))
DROP PROCEDURE GDDIANOS.BORRAR_PERSONA
...
GO
CREATE PROCEDURE GDDIANOS.BORRAR_PERSONA(@ID INT) AS 
BEGIN --TRANSAC
		
	DELETE FROM ROL_POR_USUARIO WHERE @ID IN ( 
	SELECT ROL_ROL_ID
	FROM GDDIANOS.USUARIO USUARIO JOIN GDDIANOS.PERSONA PERSONA ON (USUARIO.USUA_PERS_ID  = PERSONA.PERS_ID)
	JOIN ROL_POR_USUARIO ROL ON (USUARIO.USUA_USERNAME = ROL.ROL_USUA_USERNAME))
	
	DELETE GDDIANOS.USUARIO WHERE (@ID = USUA_PERS_ID)

	DELETE GDDIANOS.PERSONA WHERE (@ID = PERS_ID)
...
GO
CREATE PROCEDURE GDDIANOS.GET_TELEFONO(@TELEFONO INT) AS
BEGIN
	RETURN (SELECT COUNT(*)
	FROM GDDIANOS.PERSONA PERSONAS
	WHERE PERSONAS.PERS_TELEFONO = @TELEFONO)
END
