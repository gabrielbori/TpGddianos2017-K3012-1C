...
GO
CREATE PROCEDURE GDDIANOS.VIAJE_YA_REGISTRADO (@FECHA DATETIME ,@HORAI DATETIME,@CLIENTE INT) AS
IF EXISTS (SELECT VIAJE_ID FROM GDDIANOS.VIAJE 
WHERE CONVERT(DATE,@FECHA,121) = VIAJE_FECHA 
	AND CONVERT (TIME,@HORAI,121) = VIAJE_HORA_INICIO	
	AND GDDIANOS.GET_ID_PERSONA_POR_DNI(@CLIENTE) = VIAJE_CLIENTE_PERS_ID)
RETURN (0)
ELSE
RETURN (1)
...
INSERT INTO GDDIANOS.PAGO ([PAGO_FECHA]
	  ...
	  ,30
	  FROM GD_ESQUEMA.MAESTRA MIG2,GDDIANOS.PERSONA PERSONA
	  WHERE CHOFER_TELEFONO=PERS_TELEFONO
