...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.ACTUALIZAR_TIPO_PERSONA'))
DROP PROCEDURE GDDIANOS.ACTUALIZAR_TIPO_PERSONA

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.GET_ESTADO_PERFIL'))
DROP PROCEDURE GDDIANOS.GET_ESTADO_PERFIL

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.GET_CODIGO_POSTAL'))
DROP PROCEDURE GDDIANOS.GET_CODIGO_POSTAL

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.CHOFER_ACTIVO'))
DROP PROCEDURE GDDIANOS.CHOFER_ACTIVO
  ...
  BEGIN
  		...
  		BEGIN
 			--CHOFERES CON MAYOR RECAUDACION
  			...
  			FROM GDDIANOS.PERSONA JOIN GDDIANOS.PAGO ON PERS_ID=PAGO_PERS_ID
 		...
 		BEGIN
 			--CLIENTES CON MAYOR CONSUMO
  			SELECT TOP 5 PERS_NOMBRE 'Cliente Nombre', PERS_APELLIDO 'Cliente Apellido', PERS_DNI 'Cliente DNI',
 				...
 				WHERE (GDDIANOS.CALCULAR_TRIMESTRE(FACT_FECHA_INICIO)) = @TRIMESTRE AND YEAR(FACT_FECHA_INICIO) = @ANIO
  				GROUP BY PERS_NOMBRE, PERS_APELLIDO, PERS_DNI
 			...
 			--CLIENTE QUE UTILIZO MAS VECES EL MISMO AUTOMOVIL EN LOS VIAJES QUE HA REALIZADO
  			SELECT TOP 5 P1.PERS_NOMBRE 'Cliente Nombre', P1.PERS_APELLIDO 'Cliente Apellido', P1.PERS_DNI 'Cliente DNI', COCHE_PATENTE 'Automovil', COUNT((V1.VIAJE_ID)) 'Cantidad viajes'
  				FROM (GDDIANOS.VIAJE V1 JOIN GDDIANOS.PERSONA P1 ON (V1.VIAJE_CLIENTE_PERS_ID = PERS_ID)) JOIN 
 					 ...
 					 JOIN GDDIANOS.CLIENTE ON P1.PERS_ID=CLIE_PERS_ID
  				WHERE V1.VIAJE_CHOF_PERS_ID = COCHE_CHOFER AND COCHE_ESTADO = 1
 					AND GDDIANOS.CALCULAR_TRIMESTRE(V1.VIAJE_FECHA)=@TRIMESTRE AND YEAR(V1.VIAJE_FECHA)=@ANIO
...
CREATE PROCEDURE GDDIANOS.FIND_PERSONA(@NOMBRE VARCHAR(255) = NULL,@APELLIDO VARCHAR(255) = NULL, 
	@DOC VARCHAR(18) = NULL, @TIPO CHAR(7)) AS
BEGIN
...
BEGIN
	...
	SELECT PERS_ID AS ID, PERS_DNI AS Numero, 
	PERS_NOMBRE AS Nombre, PERS_APELLIDO AS Apellido, PERS_FECHA_NACIM AS 'Fecha Nacimiento'
	FROM GDDIANOS.PERSONA JOIN GDDIANOS.CHOFER ON PERS_ID=CHOF_PERS_ID
	WHERE (@NOMBRE IS NULL OR PERS_NOMBRE LIKE '%' + @NOMBRE + '%') AND
		  (@APELLIDO IS NULL OR PERS_APELLIDO LIKE '%' + @APELLIDO + '%') AND
	...
	SELECT PERS_ID AS ID, PERS_DNI AS Numero, 
	PERS_NOMBRE AS Nombre, PERS_APELLIDO AS Apellido, PERS_FECHA_NACIM AS 'Fecha Nacimiento'
	FROM GDDIANOS.PERSONA JOIN GDDIANOS.CLIENTE ON PERS_ID=CLIE_PERS_ID
	WHERE (@NOMBRE IS NULL OR PERS_NOMBRE LIKE '%' + @NOMBRE + '%') AND
		  (@APELLIDO IS NULL OR PERS_APELLIDO LIKE '%' + @APELLIDO + '%') AND
		  (@DOC IS NULL OR PERS_DNI LIKE '%' + @DOC + '%') AND
		  CLIE_ESTADO = 1		  
...
GO 
CREATE PROCEDURE GDDIANOS.BAJA_PERSONA (@ID INT, @TIPO CHAR(7))
AS 
BEGIN

	IF (@TIPO = 'Chofer')
	BEGIN
		UPDATE GDDIANOS.CHOFER SET CHOF_ESTADO = 0 WHERE (CHOF_PERS_ID = @ID)
	END
	ELSE
	BEGIN
		UPDATE GDDIANOS.CLIENTE SET CLIE_ESTADO = 0 WHERE (CLIE_PERS_ID = @ID)
	END
END
...
@MAIL VARCHAR(255), @COD VARCHAR(8), @ESTADO INT, @TIPO CHAR(7))
AS
BEGIN
	...
	IF (@TIPO = 'Chofer')
	BEGIN
	UPDATE GDDIANOS.CHOFER SET CHOF_ESTADO = @ESTADO WHERE CHOF_PERS_ID = @ID
	END
	ELSE
	BEGIN
	UPDATE GDDIANOS.CLIENTE SET CLIE_ESTADO = @ESTADO, CLIE_COD_POSTAL = @COD WHERE CLIE_PERS_ID = @ID
	END
...
@MAIL VARCHAR(255),@COD VARCHAR(8), @TIPO CHAR(7))
AS
BEGIN

	DECLARE @ID INT

	INSERT GDDIANOS.PERSONA VALUES (@TELEFONO,@NOMBRE,@APELLIDO,@DNI,@DIRECCION,@FECHA,1,@MAIL)
	SET @ID = (SELECT PERS_ID FROM GDDIANOS.PERSONA WHERE (@TELEFONO = PERS_TELEFONO AND @DNI = PERS_DNI))
	
	IF (@TIPO = 'Cliente')
	BEGIN
	INSERT GDDIANOS.CLIENTE VALUES (@ID, @COD, 1)
	END
	ELSE
	BEGIN
	INSERT GDDIANOS.CHOFER VALUES (@ID, 1)
	END
	
END

GO
CREATE PROCEDURE GDDIANOS.GET_PERSONA_POR_DNI(@DNI INT, @TIPO CHAR(7)) AS
BEGIN
	IF (@TIPO = 'Chofer')
	BEGIN
		SELECT PERS_ID
		FROM GDDIANOS.PERSONA PERSONAS JOIN GDDIANOS.CHOFER ON (PERS_ID = CHOF_PERS_ID)
		WHERE PERSONAS.PERS_DNI = @DNI
	END	
	ELSE
		BEGIN
		SELECT PERS_ID
		FROM GDDIANOS.PERSONA PERSONAS JOIN GDDIANOS.CLIENTE ON (PERS_ID = CLIE_PERS_ID)
		WHERE PERSONAS.PERS_DNI = @DNI
	END	
END
...
RETURN(
SELECT COCHE_PATENTE
FROM GDDIANOS.COCHE JOIN GDDIANOS.PERSONA ON (COCHE_CHOFER = PERS_ID) JOIN GDDIANOS.CHOFER ON (PERS_ID = CHOF_PERS_ID)
WHERE PERS_DNI = @CHOFER AND COCHE_PATENTE != @PATENTE AND COCHE_ESTADO = 1)
...
BEGIN
	...
	IF ((SELECT COUNT(*) FROM GDDIANOS.COCHE WHERE COCHE_PATENTE = @PATENTE) = 0 ) 
	INSERT GDDIANOS.COCHE VALUES (@PATENTE,'A',@ESTADO,@MARCA,@MODELO,'A',@IDCHOFER);
	ELSE
	UPDATE GDDIANOS.COCHE SET COCHE_LICENCIA = 'A', COCHE_ESTADO = @ESTADO, COCHE_MARCA_NOMBRE = @MARCA, COCHE_MODELO_CODIGO = @MODELO
	, COCHE_RODADO = 'A', COCHE_CHOFER = @IDCHOFER WHERE COCHE_PATENTE = @PATENTE
...
SELECT COCHE_PATENTE
FROM GDDIANOS.COCHE
WHERE COCHE_PATENTE = @PATENTE AND COCHE_ESTADO = 1
END
...
BEGIN		
		SELECT *
		FROM GDDIANOS.TURNO
		WHERE  @DESCRIPCION IS NULL OR TURNO_DESCRIPCION LIKE '%' + @DESCRIPCION + '%'
...
IF EXISTS(SELECT ITEM_VIAJE_ID FROM @VIAJES WHERE ITEM_VIAJE_ID
	 IN (SELECT ITEM_VIAJE_ID FROM gddianos.ITEM_FACTURA))
RETURN (select distinct item_fact_id from gddianos.item_factura where item_viaje_id=(select top 1 item_viaje_id from @viajes))
ELSE
RETURN (0)
end
...
IF EXISTS(SELECT ITEM_VIAJE_ID FROM @VIAJES WHERE ITEM_VIAJE_ID 
	IN (SELECT ITEM_PAGO_VIAJE_ID FROM gddianos.ITEM_PAGO))
RETURN (SELECT distinct ITEM_pago_numero FROM gddianos.item_pago where item_pago_viaje_id=(select top 1 ITEM_VIAJE_ID FROM @VIAJES))
ELSE
RETURN (0)
...
GO
CREATE PROCEDURE GDDIANOS.ACTUALIZAR_TIPO_PERSONA (@TELEFONO INT, @COD VARCHAR(8), @TIPO CHAR(7))
AS
BEGIN
	DECLARE @ID INT

	SET @ID = (SELECT PERS_ID FROM GDDIANOS.PERSONA WHERE (@TELEFONO = PERS_TELEFONO))
...
GO
CREATE PROCEDURE GDDIANOS.GET_ESTADO_PERFIL (@TELEFONO INT, @COD VARCHAR(8), @TIPO CHAR(7))
AS
BEGIN
	DECLARE @ID INT
	...
	IF (@TIPO = 'Chofer')
	BEGIN
		RETURN (SELECT CHOF_ESTADO FROM CHOFER WHERE CHOF_PERS_ID = @ID)
	END
	ELSE
	BEGIN
		RETURN (SELECT CLIE_ESTADO FROM CLIENTE WHERE CLIE_PERS_ID = @ID)
	END
END

GO
CREATE PROCEDURE GDDIANOS.GET_CODIGO_POSTAL (@ID INT)
AS
	SELECT CLIE_COD_POSTAL CODIGOPOSTAL
	FROM GDDIANOS.CLIENTE
	WHERE CLIE_PERS_ID = @ID

GO
CREATE PROCEDURE GDDIANOS.CHOFER_ACTIVO (@DNI INT, @NOMBRE VARCHAR(510))
AS
	RETURN (SELECT COUNT(*)
	FROM GDDIANOS.PERSONA JOIN GDDIANOS.CHOFER ON (CHOF_PERS_ID = PERS_ID)
	WHERE (PERS_DNI = @DNI) AND (PERS_NOMBRE ++ ' ' ++ PERS_APELLIDO = @NOMBRE) AND (CHOF_ESTADO = 1) )
...
INSERT INTO gddianos.PERSONA 
 SELECT DISTINCT
  ...
  SELECT PERS_ID,1 FROM GDDIANOS.PERSONA WHERE PERS_DNI IN(SELECT DISTINCT CHOFER_DNI FROM [GD1C2017].[gd_esquema].[Maestra])
...
INSERT INTO GDDIANOS.CLIENTE 
  SELECT PERS_ID,'',1 FROM GDDIANOS.PERSONA WHERE PERS_DNI IN(SELECT DISTINCT CLIENTE_DNI FROM [GD1C2017].[gd_esquema].[Maestra])
