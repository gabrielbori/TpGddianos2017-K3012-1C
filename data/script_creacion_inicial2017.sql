...
@MAIL VARCHAR(255), @ESTADO INT, @COD VARCHAR(8),@ROL_ID INT)
AS
BEGIN
	...
	PERS_ESTADO = @ESTADO, PERS_MAIL = @MAIL
	WHERE (PERS_ID = @ID)

	IF(@ROL_ID=3)
	BEGIN
	UPDATE GDDIANOS.CLIENTE SET CLIE_COD_POSTAL=@COD WHERE CLIE_PERS_ID=@ID
	END
	...
	WHERE PERS_ID = @ID)
	IF NOT EXISTS(SELECT * FROM GDDIANOS.ROL_POR_USUARIO WHERE ROL_USUA_USERNAME=@USER AND ROL_ROL_ID=@ROL_ID)
	INSERT GDDIANOS.ROL_POR_USUARIO VALUES (@USER, @ROL_ID)
...
BEGIN	
	...
	IF(@TIPO=2)
	BEGIN
		INSERT INTO GDDIANOS.CHOFER VALUES(SCOPE_IDENTITY())
	END
	IF(@TIPO=3)
	BEGIN
		INSERT INTO GDDIANOS.CLIENTE VALUES(SCOPE_IDENTITY(),@COD)
	END
