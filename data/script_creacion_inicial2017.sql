...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.ROL_POR_NOMBRE') AND OBJECTPROPERTY(id, 'IsUserTable') = 1)
DROP PROCEDURE GDDIANOS.ROL_POR_NOMBRE

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('GDDIANOS.GET_FUNCIONALIDADES') AND OBJECTPROPERTY(id, 'IsUserTable') = 1)
...
BEGIN
	...
	WHERE (@HI BETWEEN TURNO_HORA_INICIO AND TURNO_HORA_FIN) OR (@HF BETWEEN TURNO_HORA_INICIO AND TURNO_HORA_FIN)

	IF (@CANT = 0)
		INSERT GDDIANOS.TURNO VALUES (@HI,@HF,@DESCRIPCION,@VALOR,@PRECIO,1)
	ELSE
		PRINT('No se realizó el alta, ya que el horario de inicio o fin del nuevo turno se encuentra incluido en otro');
END
...
AS
BEGIN
	SELECT TURNO_DESCRIPCION FROM GDDIANOS.TURNO 
	RETURN 1
END
...
AS
BEGIN
	UPDATE GDDIANOS.TURNO SET TURNO_HORA_INICIO = @HS, TURNO_HORA_FIN = @HF, TURNO_DESCRIPCION =@DESCRIPCION, TURNO_VALOR_KM =@VALOR,
